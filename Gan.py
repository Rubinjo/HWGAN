...
from OCR import make_recognizer_model
...
recognizer = make_recognizer_model()

# run D + R on image generated by G
gan_output_d = discriminator(generator.output, training=True)
gan_output_r = recognizer([generator.output, generator.input[1], input_length, label_length], training=True)

# This method returns a helper function to compute cross entropy loss
cross_entropy = tf.keras.losses.BinaryCrossentropy(from_logits=True)
cross_entropy_class = tf.keras.losses.BinaryCrossentropy(from_logits=True)
...
def recognizer_loss(real_output, fake_output):
    real_loss = cross_entropy(tf.ones_like(real_output), real_output)
    fake_loss = cross_entropy(tf.zeros_like(fake_output), fake_output)
    total_loss = real_loss + fake_loss
    return total_loss
...
recognizer_optimizer = tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE)
